...
bool clean_environment(){
	...
	return true;
#else
	printf(COLOR_YELLOW"the requrested operation is currently not supported for non-linux Operating Systems\n"COLOR_RESET);
	return false;
#endif
}
bool init_test(){
	if(!clean_environment()){
		printf(COLOR_YELLOW"an error occured when cleaning the environment, you may encounter an unaccurate reports\n"COLOR_RESET);
	}
...
static bool entries_are_equal(const IntVaultEntry *e1, const IntVaultEntry *e2) {
    ...
    if (e1->created_at != e2->created_at) return false;
    if (e1->updated_at != e2->updated_at) return false;
    if (e1->uuid == NULL || e2->uuid == NULL) {
        if (e1->uuid != e2->uuid) return false;
    } else {
        if (strcmp(e1->uuid, e2->uuid) != 0) return false;
    }

    if (!compare_blob(e1->service_name, e1->service_len, e2->service_name, e2->service_len)) return false;
    if (!compare_blob(e1->username, e1->username_len, e2->username, e2->username_len)) return false;
    if (!compare_blob(e1->password, e1->password_len, e2->password, e2->password_len)) return false;
    if (!compare_blob(e1->notes, e1->notes_len, e2->notes, e2->notes_len)) return false;
...
static void print_entry(IntVaultEntry *entry){
    ...
    printf("| %-14s | %-40.*s |\n", "Service", (int)entry->service_len, (char*)entry->service_name);
...
static void print_linked_list_entries(const DLinkedList *list) {
    ...
    while (current != NULL) {
        ...
        printf("| %-14s | Idx: %-3d | Curr: %p                      |\n", "== NODE ==", index, (void*)current);
        printf("| %-14s | Prev: %p | Next: %p                   |\n", "", (void*)current->prev, (void*)current->next);
        printf("|----------------|------------------------------------------------------|\n");
        if (entry) {
            printf("| %-14s | %-52s |\n", "UUID", 
                   entry->uuid ? entry->uuid : "(null)");
            printf("| %-14s | (len:%-3u) %-42.*s |\n", "Service", 
                   entry->service_len, (int)entry->service_len, (char*)entry->service_name);
            printf("| %-14s | (len:%-3u) %-42.*s |\n", "Username", 
                   entry->username_len, (int)entry->username_len, (char*)entry->username);
            printf("| %-14s | (len:%-3u) %-42.*s |\n", "Password", 
                   entry->password_len, (int)entry->password_len, (char*)entry->password);
            printf("| %-14s | (len:%-3u) %-42.*s |\n", "Notes", 
                   entry->notes_len, (int)entry->notes_len, (char*)entry->notes);
